...
//    }
    ...
    private void setup(PocketsConfig pocketsConfig) {
...
//        this.table.setupBalls(balls);
    ...
    private void init(String colourName, double friction, long dimX, long dimY, PocketsConfig pocketsConfig) {
        ...
        for (PocketConfig pConfig : pocketsConfig.getPockets()) {
            this.pockets.add(new Pocket(pConfig.getPositionConfig().getX(), pConfig.getPositionConfig().getY(), pConfig.getRadius()));
        }
    }
    ...
    public void addToGroup(ObservableList<Node> groupChildren) {
        ...
        for (Ball ball : this.balls) {
            ball.addCueStickToGroup(groupChildren);
        }
    }
    ...
    /**
     * Reverts the balls states back to the current memento
     */
    public void undo() {
        for (Ball ball : this.balls) {
            ball.restoreState();
