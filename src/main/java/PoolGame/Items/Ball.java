 ...
 */
public class Ball implements Drawable, Movable, Subject, Savable {
    ...
    private int value;
    private BallMemento ballMemento = null;
    ...
    public Ball(Ball ball) throws IllegalArgumentException {
        this.shape = new Circle(ball.getXPos(), ball.getYPos(), ball.getRadius());
        this.cueStick = ball.cueStick;
        this.setColour(String.valueOf(ball.colour));
     ...
     * @return A Color object that the ball has
     */
    ...
    public void addCueStickToGroup(ObservableList<Node> groupChildren) {
        groupChildren.add(this.cueStick.getRectangle());
        groupChildren.add(this.cueStick.getCircle());
    }
    ...
    /**
     * Set the score value of this ball
    ...
    /**
     * Get the score value of the ball
     * @return score value of the ball
     */
    public int getValue() {
        return value;
    }
        ...
        this.shape.setOnMouseClicked(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    ...
                    this.cueStick.startCueStick(this.shape.getCenterX(), this.shape.getCenterY(), this);
                }
            }
        );
    }
     ...
     */
    Point2D calculateCueBallVelOnHit(double cursorPosX, double cursorPosY) {
    ...
    */
    ...
    /**
     * Returns the cueStick object the ball holds
     * @return CueStick
     */
    ...
    /**
     * Attaches an observer to the ball. This observer will be used to notify the timer and the scoreboard
    ...
    /**
     * Calls the observers of the ball to use their update function.
     */
    @Override
    public void alert() {
    ...
    public void saveState() {
        this.ballMemento = new BallMemento(this.getXPos(), this.getYPos(), this.getXVel(), this.getYVel(), (BallPocketStrategy) this.getPocketAction().clone(), this.isDisabled(), this.getFallCounter());
    }

    @Override
    public void restoreState() {
        if (this.ballMemento == null) {
            return;
        }
        this.setXPos(this.ballMemento.getPos()[0]);
        this.setYPos(this.ballMemento.getPos()[1]);
        this.setXVel(this.ballMemento.getVel()[0]);
        this.setYVel(this.ballMemento.getVel()[1]);
        this.setPocketAction(this.ballMemento.getPocketAction());
        if (this.ballMemento.isDisabled()) {
            this.disabled = true;
            this.shape.setVisible(false);
        } else {
            this.disabled = false;
        ...
        this.fallCounter = this.ballMemento.getFallCounter();

    }

    private static class BallMemento implements Memento {
        private final double[] pos;
        private final double[] vel;
        private final BallPocketStrategy pocketAction;
        private final boolean disabled;
        private final int fallCounter;

        public BallMemento(double xPos, double yPos, double xVel, double yVel, BallPocketStrategy pocketAction, boolean disabled, int fallCounter) {
            this.pos = new double[]{xPos, yPos};
            this.vel = new double[]{xVel, yVel};
            this.disabled = disabled;
            this.fallCounter = fallCounter;
            this.pocketAction = pocketAction;
        }

        private BallPocketStrategy getPocketAction() {
            return pocketAction;
        }
